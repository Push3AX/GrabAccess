 ...
 */
int wim_header ( struct vfat_file *file, struct wim_header *header ) {

  /* Sanity check */
  if ( sizeof ( *header ) > file->len ) {
    printf ( "WIM file too short (0x%lx bytes)\n", (unsigned long)file->len );
    return -1;
  }
 ...
 * @ret rc    Return status code
 */
static int wim_chunk_offset ( struct vfat_file *file,
  ...
  /* Sanity check */
  if ( chunks_len > zlen ) {
    printf ( "Resource too short for %d chunks\n", chunks );
    return -1;
  }
  ...
  *offset = ( chunks_len + ( ( offset_len == sizeof ( u.offset_64 ) ) ?
  ...
  if ( *offset > zlen ) {
    printf ( "Chunk %d offset lies outside resource\n", chunk );
    return -1;
  }
  return 0;
}
 ...
 * @ret rc    Return status code
 */
static int wim_chunk ( struct vfat_file *file, struct wim_header *header,
           struct wim_resource_header *resource,
  ...
  if ( len == expected_out_len ) {
    ...
    if ( ( ( size_t ) out_len ) != expected_out_len ) {
      printf ( "Unexpected output length 0x%lx (expected 0x%lx)\n",
            out_len, (unsigned long)expected_out_len );
      return -1;
    }
 ...
 * @ret rc    Return status code
 */
int wim_read ( struct vfat_file *file, struct wim_header *header,
  ...
  if ( ( resource->offset + zlen ) > file->len ) {
    printf ( "Resource exceeds length of file\n" );
    return -1;
  }
 ...
 * @ret rc    Return status code
 */
int wim_count ( struct vfat_file *file, struct wim_header *header,
    unsigned int *count ) {
  ...
  for ( offset = 0 ; ( offset + sizeof ( entry ) ) <= header->lookup.len ;
    ...
    if ( entry.resource.zlen__flags & WIM_RESHDR_METADATA ) {
      (*count)++;
      printf ( "...found image %d metadata at +0x%lx\n",
 ...
 * @ret rc    Return status code
 */
int wim_metadata ( struct vfat_file *file, struct wim_header *header,
  ...
  printf ( "Cannot find WIM image index %d in %s\n", index, file->name );
  return -1;
}
 ...
 * @ret rc    Return status code
 */
static int wim_direntry ( struct vfat_file *file, struct wim_header *header,
 ...
 * @ret rc    Return status code
 */
int wim_path ( struct vfat_file *file, struct wim_header *header,
 ...
 * @ret rc    Return status code
 */
int wim_file ( struct vfat_file *file, struct wim_header *header,
         struct wim_resource_header *meta, const wchar_t *path,
         struct wim_resource_header *resource ) {
  struct wim_directory_entry direntry;
  struct wim_lookup_entry entry;
 ...
 * @ret rc    Return status code
 */
int wim_dir_len ( struct vfat_file *file, struct wim_header *header,
