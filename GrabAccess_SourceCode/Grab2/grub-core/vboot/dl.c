...
void* p_text_section_addr;

// where to load vbootcore.mod module
extern char *vboot_core_addr;
...
int EXPORT_FUNC(vboot_dl_unload) (grub_dl_t mod);
void vboot_dl_unload_unneeded (void);
void vboot_dl_unload_all (void);
void EXPORT_FUNC(vboot_dl_iterate) (int (*hook) (grub_dl_t mod));
...
static grub_dl_list_t grub_dl_head_old;

static grub_dl_t
vboot_dl_get (const char *name);
...
grub_dl_remove (grub_dl_t mod)
{
  ...
  for (p = &grub_dl_head_old, q = *p; q; p = &q->next, q = *p)
    if (q->mod == mod)
      {
	*p = q->next;
	grub_free (q);
...
vboot_dl_get (const char *name)
{
  ...
  for (l = grub_dl_head_old; l; l = l->next)
...
void
vboot_dl_iterate (int (*hook) (grub_dl_t mod))
{
  grub_dl_list_t l;
...
static grub_err_t
vboot_dl_register_symbol (const char *name, void *addr, grub_dl_t mod)
{
  grub_symbol_t sym;
  unsigned k;
...
grub_dl_check_header (void *ehdr, grub_size_t size)
{
  ...
  if (vboot_arch_dl_check_header (ehdr)
...
grub_dl_load_segments (grub_dl_t mod, const Elf_Ehdr *e)
{
  ...
  Elf_Shdr *s1;
  const char *str;
  grub_int32_t last_seg_size = 0;
  char *addr = NULL;

  s1 = (Elf_Shdr *) ((char *) e + e->e_shoff + e->e_shstrndx * e->e_shentsize);
  str = (char *) e + s1->sh_offset;
      ...
      if (s->sh_flags & SHF_ALLOC)
	{
	  ...
	  if (s->sh_size)
	    {		  
		  ...
		  {
			if (addr == NULL)
				addr = vboot_core_addr;
			else
			{
				addr += last_seg_size;
				addr = (char *)roundUp((grub_uint32_t)addr, 0x10000); /* align to 64KiB */
			}
		  }
		  else
		  {
	      ...
	      if (! addr)
		{
		  ...
		  grub_dprintf("modules", "loading segment '%s' [0x%x bytes] at %p\n", str + s->sh_name, s->sh_size, addr);
        if (!grub_strcmp(str + s->sh_name, ".text"))
		{
			p_text_section_addr = addr;
		}
  ...
  for (i = 0;
	...
	case STT_OBJECT:
	  sym->st_value += (Elf_Addr) grub_dl_get_section_addr (mod,
								sym->st_shndx);
	  if (bind != STB_LOCAL)
	    if (vboot_dl_register_symbol (name, (void *) sym->st_value, mod))
	      return grub_errno;
	  break;
	...
	case STT_FUNC:
	  ...
	  if (grub_strcmp (name, "vbootcore_init"/*"grub_mod_init"*/) == 0)
...
int
vboot_dl_ref (grub_dl_t mod)
{
  grub_dl_dep_t dep;

  for (dep = mod->dep; dep; dep = dep->next)
    vboot_dl_ref (dep->mod);

  return ++mod->ref_count;
}

static int
vboot_dl_unref (grub_dl_t mod)
{
  grub_dl_dep_t dep;

  for (dep = mod->dep; dep; dep = dep->next)
    vboot_dl_unref (dep->mod);
...
static grub_dl_t
vboot_dl_load_core (void *addr, grub_size_t size)
{
  Elf_Ehdr *e;
  grub_dl_t mod;
  ...
  if (grub_dl_resolve_name (mod, e)
      ...
      || vboot_arch_dl_relocate_symbols (mod, e))
    {
      mod->fini = 0;
      vboot_dl_unload (mod);
      return 0;
  ...
  /* grub_dl_call_init (mod); we need to invoke this from our bootloader */
...
grub_dl_t
vboot_dl_load_file (const char *filename)
{
  ...
  mod = vboot_dl_load_core (core, size);
  if (! mod)
    {
      grub_free (core);
      return 0;
...
grub_dl_t
vboot_dl_load (const char *name)
{
  char *filename;
  grub_dl_t mod;
  const char *grub_dl_dir = grub_env_get ("prefix");

  mod = vboot_dl_get (name);
  if (mod)
    return mod;
  ...
  mod = vboot_dl_load_file (filename);
  grub_free (filename);
  ...
  for (dep = mod->dep; dep; dep = depn)
    {
      ...
      if (! vboot_dl_unref (dep->mod))
	vboot_dl_unload (dep->mod);
...
void
vboot_dl_unload_unneeded (void)
{
  /* Because grub_dl_remove modifies the list of modules, this
     implementation is tricky.  */
  grub_dl_list_t p = grub_dl_head_old;

  while (p)
    {
      if (vboot_dl_unload (p->mod))
	{
	  p = grub_dl_head_old;
	  continue;
	}
...
/* Unload all modules.  */
void
vboot_dl_unload_all (void)
{
  while (grub_dl_head_old)
    {
      grub_dl_list_t p;

      vboot_dl_unload_unneeded ();

      /* Force to decrement the ref count. This will purge pre-loaded
	 modules and manually inserted modules.  */
      for (p = grub_dl_head_old; p; p = p->next)
	p->mod->ref_count--;
    }
}
